...
function formatFrontendCourse(course: any) {
  ...
  const totalVacancies = course.classes ? course.classes.reduce((sum: number, cls: any) => sum + cls.vacancies, 0) : 0;
  const enrolledCount = course.classes ? course.classes.reduce((sum: number, cls: any) => sum + cls.enrolledCount, 0) : 0;
  ...
  return {
    ...
    formadores: course.instructors ? course.instructors.map((i: any) => formatInstructor(i.instructor)) : [],
    modulos: course.modules ? course.modules.map((m: any) => ({
      id: m.id,
      title: m.title,
      duration: m.duration,
      topics: m.topics,
...
export const courseService = {
  ...
  async update(id: string, data: UpdateCourseInput) {
    ...
    if (!existingCourse) throw new Error("Curso não encontrado.");
