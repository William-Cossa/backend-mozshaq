...
function formatFrontendPayment(payment: any) {
...
export const paymentService = {
  ...
  async review(id: string, data: ReviewPaymentInput) {
    const payment = await prisma.payment.findUnique({ where: { id } });
    if (!payment) throw new Error("Pagamento não encontrado.");
    if (payment.status !== "PENDING") throw new Error("Apenas pagamentos pendentes podem ser revisados.");
